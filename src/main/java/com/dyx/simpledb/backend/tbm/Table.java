...
public class Table {
    ...
    public void insert(long xid, InsertObj insertObj) throws Exception {
        // Map<String, Object> entry = string2Entry(insertObj.values);
    ...
    private Map<String, Object> string2Entry(InsertObj insertObj) {
        ...
        //判断字段表是否为空，若不为0表示需要插入所有字段
        if (insertObj.fields.length == 0){
            for (int i = 0; i < fields.size(); i++) {
                Field f = fields.get(i);
                Object v = f.string2Value(insertObj.values[i]);
        ...
        int count = 0;
        //若字段列表不为0，表示只需要插入指定字段，其他采用默认值
        for (String fieldType : insertObj.fields) {
            //已有字段对其进行匹配
            for (int i = 0; i < fields.size(); i++) {
                // 如果匹配成功，则转换为对应的map映射
                Field field = fields.get(i);
                if (field.fieldName.equals(fieldType)){
                    Object v = field.string2Value(insertObj.values[count++]);
                    entry.put(field.fieldName, v);
                    continue;
                }
                entry.put(field.fieldName, field.defaultValue);
            }
        }
